    ...
    if (!AssetPath.IsEmpty())
    {
        ...
        if (ModType == "Map")
        {
            ...
            for (TActorIterator<AActor> ActorItr(LoadedWorld); ActorItr; ++ActorItr)
            {
                ...
                // Automatically add mesh path if this is a StaticMeshActor
                if (AStaticMeshActor* SMActor = Cast<AStaticMeshActor>(Actor))
                {
                    ...
                    {
                        FString OriginalPath = MeshComp->GetStaticMesh()->GetPathName();
                        ReferencedMeshes.Add(OriginalPath);

                        FString MeshName = FPaths::GetBaseFilename(OriginalPath);
                        FString VirtualPath = FString::Printf(TEXT("/Mods/%s/Models/%s.%s"), *ModName, *MeshName, *MeshName);

                        // ✅ This was missing
                        ActorJson->SetStringField(TEXT("AssetDirectory"), VirtualPath);
                    }
                }
            ...
            // Create models folder and copy all referenced cooked assets
            FString ModelsFolder = OutputDir / TEXT("Models");
            IFileManager::Get().MakeDirectory(*ModelsFolder, true);
        }
